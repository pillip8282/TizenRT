...
// boot file name 64
	...
	printf("option %d length %d type %d\n", dhcpmsg->options[0], dhcpmsg->options[1], dhcpmsg->options[2]);
	printf("dhcp magic %02x %02x %02x\n", dhcpmsg->options[192], dhcpmsg->options[193], dhcpmsg->options[194]);
...
static uint8_t g_null_addr[4] = {0x0, 0x0, 0x0, 0x0};
static uint8_t g_dhcp_magic[4] = {0x63, 0x82, 0x53, 0x63};
//static uint8_t g_dhcp_lease_addr[4] = {0x03, 0x01, 0xa8, 0xc0};
...
void vwifi_send_dhcpoffer(uint8_t *buf, uint32_t len)
{
	...
	memcpy(dhcpmsg->ciaddr, g_null_addr, 4);
	memcpy(dhcpmsg->yiaddr, g_null_addr, 4);
	memcpy(dhcpmsg->siaddr, g_null_addr, 4);
	memcpy(dhcpmsg->giaddr, g_null_addr, 4);
...
void vwifi_handle_packet(uint8_t *buf, uint32_t len)
{
	...
	if (next == 1) {
		VWIFI_LOG("[pkbuild] send offer message\n");
		vwifi_send_dhcpoffer(buf, len);
	} else if (next == 3) {
		VWIFI_LOG("[pkbuild] send ACK message\n");
