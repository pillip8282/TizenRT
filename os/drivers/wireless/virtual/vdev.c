...
{
	struct vwifi_msg *tmsg = (struct vwifi_msg *)malloc(sizeof(struct vwifi_msg));
	if (!tmsg) {
		VWIFI_ERROR(0);
		return -1;
	}
	sem_t *signal = (sem_t *)malloc(sizeof(sem_t));
	if (!signal) {
		free(tmsg);
		VWIFI_ERROR(0);
		return -1;
...
{
	lldbg("[pkbuild] %p %d\t%s%d\n", buf, buflen, __FUNCTION__, __LINE__);
	int sent = 0;
	while (1) {
...
struct netdev* vdev_register_dev(int sizeof_priv)
{
	...
	nconfig.hwaddr[0] = 0x00;
	nconfig.hwaddr[1] = 0x04;
	nconfig.hwaddr[2] = 0x96;
	nconfig.hwaddr[3] = 0x1d;
	nconfig.hwaddr[4] = 0xbe;
	nconfig.hwaddr[5] = 0xb0;
...
{
	VWIFI_LOG("[pkbuild] send packet device %p\n", buf);
	if (!g_vwifi_dev) {
		VWIFI_ERROR(0);
		return;
	}
	VWIFI_LOG("[pkbuild] send packet %d\n", len);
