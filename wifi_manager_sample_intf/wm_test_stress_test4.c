 ...
 * callbacks
 */
static void wm_cb_sta_connected(wifi_manager_result_e);
static void wm_cb_sta_disconnected(wifi_manager_disconnect_e);
static void wm_cb_softap_sta_join(void);
static void wm_cb_softap_sta_leave(void);
static void wm_cb_scan_done(wifi_manager_scan_info_s **, wifi_manager_scan_result_e);
...
void wm_cb_sta_connected(wifi_manager_result_e res)
{
	WT_LOG(TAG, "--> res(%d)", res);
	WM_TEST_SIGNAL;
}

void wm_cb_sta_disconnected(wifi_manager_disconnect_e res)
{
	WT_LOG(TAG, "--> res(%d) ", res);
	WM_TEST_SIGNAL;
}

void wm_cb_softap_sta_join(void)
{
	WT_LOG(TAG, "--> %s:%d", __FUNCTION__, __LINE__);
	WM_TEST_SIGNAL;
}

void wm_cb_softap_sta_leave(void)
{
	WT_LOG(TAG, "--> %s:%d", __FUNCTION__, __LINE__);
	WM_TEST_SIGNAL;
}

void wm_cb_scan_done(wifi_manager_scan_info_s **scanlist, wifi_manager_scan_result_e res)
{
	WT_LOG(TAG, "--> res(%d)", res);
	/* Make sure you copy the scan results onto a local data structure.
	 * It will be deleted soon eventually as you exit this function.
	 */
	if (res != WIFI_SCAN_SUCCESS || scanlist == NULL) {
		WM_TEST_SIGNAL;
		return;
	}
	wt_print_scanlist(*scanlist);
	WM_TEST_SIGNAL;
}
