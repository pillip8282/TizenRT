...
static void _wt_scan_done(wifi_manager_scan_info_s**, wifi_manager_scan_result_e);
...
/*  callback */
static void _wt_sta_connected(wifi_manager_result_e res)
{
	WT_LOG(TAG, "-->res(%d)", res);
	WT_TEST_SIGNAL;
}

static void _wt_sta_disconnected(wifi_manager_disconnect_e res)
{
	WT_LOG(TAG, "--> res %d", res);
	WT_TEST_SIGNAL;
}
...
static void _wt_scan_done(wifi_manager_scan_info_s**scanlist, wifi_manager_scan_result_e res)
{
	WT_LOG(TAG, "-->");
	...
	if (g_scan_join == 0) {
		 ...
		 */
		g_scanned_result = _wt_get_scanned_list(*scanlist,
												g_scanned_ssid,
												&g_scanned_auth_type,
